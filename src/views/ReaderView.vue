...
import { formatPageLabel, formatPercent } from '@/utils/format'
import { resolveTurnProfile, turnProfileDataset } from '@/utils/turnProfile'
...
/** Fine pointer → dense bar chrome; coarse-only → float overlay (orthogonal to width). */
const barChrome = computed(() => turnProfile.value.barChrome)
const floatChrome = computed(() => !barChrome.value)
const viewportWide = computed(() => turnProfile.value.dualEligible)
const turnDataAttrs = computed(() => turnProfileDataset(turnProfile.value))
...
watch(chromeVisible, async () => {
  await nextTick()
  // Bar chrome changes stage size; float chrome changes PDF fit insets.
...
watch(
  ...
  (mode, prev) => onAppearanceModeChange(mode, prev),
)

/** Engine-relevant settings — avoid deep watch thrash on unrelated UI fields. */
watch(
  () => {
...
function refreshPointerMode() {
  const next = resolveTurnProfile()
  const dualChanged = next.dualEligible !== turnProfile.value.dualEligible
  turnProfile.value = next
  if (dualChanged) applySettings()
}
...
<template>
  <div
    ...
    :data-device="turnDataAttrs.device"
    :data-anim="turnDataAttrs.anim"
    :data-pointer="turnDataAttrs.pointer"
    :style="fitInsetStyle"
  >
    <header
      v-if="!floatChrome"
